angular.module('mainApp', ['ngMaterial', 'directivelibrary', '12thdirective', 'uiMicrokernel',
    ...
    .controller('AppCtrlAdd', function($scope, $state, $objectstore, $location,$mdDialog, $window, $objectstore, $auth, $q,$http, $compile, $timeout, $mdToast, $rootScope,invoiceDetails) {
       ...
       /*_________________________________________history_________________________________________*/
        $scope.history = function(ev) { //payment history dialog box
            $mdDialog.show({
        ...
        $scope.selectedItemChange=function(name){
            ...
            client.onGetMany(function(data) {
                if (data) {
                         $scope.TDInvoice.updateInvoice=data;
                         for(var i=0;i<data.length;i++){
                             for(var j=0; j< $scope.TDInvoice.updateInvoice[i].MultiDueDAtesArr.length;j++){
                            if(name==null){
            ...
            client.getByFiltering("*");
               var client = $objectstore.getClient("advancedPayment");
               client.onGetMany(function(data) {
              if (data) {
                $scope.paymentDetails = data;
                   for(var i=0;i<$scope.paymentDetails.length;i++){
                   if($scope.paymentDetails[i].customer==name){
                ...
                $mdDialog.show($mdDialog.alert()
                            .parent(
                                angular.element(
                                    document.body))
                            .content(
                                'Unapplied advances USD'+' ' + $scope.payment.namount +' '+'This amount can be applied on future invoices.'
                            )
                            .ariaLabel('')
                            .ok('OK')
                            .targetEvent(
                ...
                client1.insert($scope.advancedPayment,{ //insertng Unapplied Advances details into advancedPayment class
                 ...
                 $scope.uInvoice(); //updating invoice as paid 
                  ...
                  $mdDialog.show($mdDialog.alert()
                   ...
                   .title('warning')
                   .content(
                       'Total payments applied exceeds the total available'
                   )
                  .ariaLabel('Alert Dialog Demo')
                  .ok('OK'))
                }       
